...
int main(int argc, char** argv) {
    int prod_count, cons_count, err_code;
    ...
    if ((err_code = pthread_mutex_init(&mutex, NULL)) != 0) {
        fprintf(stderr, "Error: pthread_mutex_init() is failed with %s\n", strerror(err_code));
        return -1;
    }
    if((err_code = pthread_cond_init(&cond, NULL)) != 0) {
        fprintf(stderr, "Error: pthread_cond_init() is failed with%s\n", strerror(err_code));
        return -1;
    }

    MyMsgQueue myMsgQueue(mutex, cond);
